...
if (isset($_FILES['file']['name'])) {
    ...
    $filename = "PROFILE_PHOTO_" . $id_card . "." . $ext;
    /* Ubicacion */

    $location = $_SERVER['DOCUMENT_ROOT'] . "/admin/img/profileUser/" . $filename;
    ...
    if (in_array(strtolower($imageFileType), $valid_extensions)) {
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], $location)) {
            ...
            $query = $conn->connect()
                ->prepare('CALL update_photo_user(:idcard, :url)');
            /* Ejecucion del script sql */
            $response =  $query->execute([
                'idcard' => $id_card,
                'url' => "/admin/img/profileUser/" . $filename
            ]);
        }
    }
